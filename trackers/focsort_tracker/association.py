...
def iou_batch_A(bboxes1, bboxes2):
    """
    From SORT: Computes IOU between two bboxes in the form [x1,y1,x2,y2]
    """
    bboxes2 = np.expand_dims(bboxes2, 0)
...
def ciou_batch_with_area(bboxes1, bboxes2):
    """
    :param bbox_p: predict of bbox(N,4)(x1,y1,x2,y2)
    :param bbox_g: groundtruth of bbox(N,4)(x1,y1,x2,y2)
    :return:
    ...
    return (ciou + 1) / 2.0, area1/area2  # resize from (-1,1) to (0,1)
...
def associate(detections, trackers, iou_threshold, velocities, previous_obs, vdc_weight):    
    ...
    # iou_matrix = ct_dist(detections, trackers)
    ...
    score_diff = detections[:, 4][..., None] * previous_obs[:, 4][None]
    if len(detections) > 0 and len(trackers)>0:
        tb_diff = iou_batch(detections, trackers) * tb_dist(detections, trackers)
        angle_diff_cost += tb_diff
