...
def show(gt_cur, pred_cur, img, trajs):
    ...
    for value, index, bbox in zip(max_value, max_index, pred_cur):
        ...
        if value > 0.6:
            ...
            if y_min < 30:
                img = cv2.putText(img, '%d_%d'%(id, id_gt), (int(x_min), int(y_max)), cv2.FONT_HERSHEY_PLAIN, 2, rgb, 2, cv2.LINE_AA)
            else:
                img = cv2.putText(img, '%d_%d'%(id, id_gt), (int(x_min), int(y_min)), cv2.FONT_HERSHEY_PLAIN, 2, rgb, 2, cv2.LINE_AA)
        else:
        ...
        if gt_cur[index, 1] not in viewed_gt:
            viewed_gt.append(gt_cur[index, 1])
...
if __name__ == "__main__":
    ...
    ours = "YOLOX_outputs/yolox_dttrainandval_ch_mot20_1080p_dtbc03_delay_dead/yolox_dttrainandval_ch_mot20_1080p_dtbc03_delay_dead_val" # preds
    # bet = 'YOLOX_outputs/yolox_dttrainandval_ch_bot_sort'

    seqs = os.listdir(gt_src)
    for seq in ['dancetrack0019']: # seqs:
        os.makedirs(os.path.join("traj_plots/{}".format(seq)), exist_ok=True)
        gt = np.loadtxt(os.path.join(gt_src, seq, "gt/gt.txt"), delimiter=",")
        pred = np.loadtxt(os.path.join(ours, "{}.txt".format(seq)), delimiter=",")
        # gt = np.loadtxt(os.path.join(bet, "{}.txt".format(seq)), delimiter=",")
        # pred = np.loadtxt(os.path.join(ours, "{}.txt".format(seq)), delimiter=",")
        ...
        while True:
            ...
            gt_cur = gt[gt[:, 0] == frame_id]
            pred_cur = pred[pred[:, 0] == frame_id]

            img = cv2.imread(img_p)

            img = show(gt_cur, pred_cur, img, trajs)
