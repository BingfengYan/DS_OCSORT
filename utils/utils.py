...
def write_results_no_score2(filename, results):
    save_format = '{frame},{id},{x1},{y1},{w},{h},{m},-1,-1,-1\n'
    with open(filename, 'w') as f:
        for frame_id, tlwhs, track_ids, mm in results:
            for tlwh, track_id, m in zip(tlwhs, track_ids, mm):
                if track_id < 0:
                    continue
                x1, y1, w, h = tlwh
                line = save_format.format(frame=frame_id, id=track_id, x1=round(x1, 1), y1=round(y1, 1), w=round(w, 1), h=round(h, 1), m=round(m, 1))
                f.write(line)
    logger.info('save results to {}'.format(filename))
